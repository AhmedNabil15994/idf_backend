...
<div class=" hide-inputs" id="project-input" style="display: {{$slider->type == 'project' ? 'block' : 'none'}}">
    {!! field()->select('project_id',__('slider::dashboard.sliders.form.project') , pluckModelsCols($projects->get(),'title','id',true), null , ['id' => 'single']) !!}
</div>

<div class=" hide-inputs" id="link-input"
     style="display: {{!$slider->type || $slider->type == 'link' ? 'block' : 'none'}}">
...
<div id="dates_container" style="display: none;">
    ...
    $slider->start_date ? \Carbon\Carbon::parse($slider->start_date)->format('Y-m-d'): null) !!}
    {!! field()->date('end_date', __('slider::dashboard.sliders.form.end_date'),
    $slider->end_date ? \Carbon\Carbon::parse($slider->end_date)->format('Y-m-d'): null) !!}
</div>

{!! field()->number('order', __('slider::dashboard.sliders.form.order')) !!}
{!! field()->file('image', __('slider::dashboard.sliders.form.image'), $slider->getFirstMediaUrl('images')) !!}
